...
std::string JavaScriptExtensions::GetBasicExtension(bool IsRecord)
{
    ...
    if(IsRecord)
    {
        ...
        "{"
            ...
            "{"
                ...
                "var elementsFromPointTarget = document;"
                "try"
                "{"
                    "if(el_target.getRootNode() instanceof ShadowRoot)"
                    "{"
                        "elementsFromPointTarget = el_target.getRootNode();"
                    "}"
                "}catch(e)"
                "{"

                "}"
                "var elements = elementsFromPointTarget.elementsFromPoint(x,y);"
                "if(elements && elements.length > 0)"
                "{"
                    "if(elements[0].shadowRoot != null)"
                    "{"
                        "position = 0;"
                    "}"
                "}"
            ...
            "if(el){"
                ...
                "}"
                "label += css;"
                "let scrolling_element = _BAS_HIDE(BrowserAutomationStudio_GetScrollingNode)();"
                "return [parseInt(rect.left),parseInt(rect.top),parseInt(rect.width),parseInt(rect.height),label,css,css2,css3,match,xpath,x + scrolling_element.scrollLeft,y + scrolling_element.scrollTop,true,is_frame,frame_element,x_with_padding,y_with_padding,position];"
            "}else{"
        ...
        "{"
            ...
            "try{"
                "for(var i = 0;i<data.length;i++){"
                    ...
                    "var shadow_index = selector.lastIndexOf('>SHADOW>');"

                    "if(shadow_index >= 0)"
                    "{"
                        "shadow_prefix = selector.substring(0,shadow_index + 8);"
                    "}"
    ...
    "{"
        ...
        "}"
        "let element_selection = window.getSelection().toString();"
    ...
    "{"
        ...
        "try{"
            ...
            "if(x > scrolling_element.scrollLeft && x < scrolling_element.scrollLeft + window.innerWidth && y > scrolling_element.scrollTop && y < scrolling_element.scrollTop + window.innerHeight)"
            "{"
                "var res = scrolling_element.scrollLeft + ',' + scrolling_element.scrollTop + ',' + x.toString() + ',' + y.toString();"
                "return res;"
            "}"
            "if(typeof(allowoutofbounds) == 'undefined' || !allowoutofbounds || x >= 0)"
            "{"
                "scrolling_element.scrollLeft = x - window.innerWidth/2;"
            "}"
            "if(typeof(allowoutofbounds) == 'undefined' || !allowoutofbounds || y >= 0)"
            "{"
                "scrolling_element.scrollTop = y - window.innerHeight/2;"
            "}"
        "}catch(e)"
        "{"
        ...
        "{"
            "try{"
                "x-=scrolling_element.scrollLeft;"
    ...
    "{"
        "_BAS_HIDE(BrowserAutomationStudio_GetScrollingNode)().scrollTop-=100;"
    "};"
    ...
    "{"
        "_BAS_HIDE(BrowserAutomationStudio_GetScrollingNode)().scrollLeft+=100"
    "};"
    ...
    "{"
        "_BAS_HIDE(BrowserAutomationStudio_GetScrollingNode)().scrollLeft+=10000"
    "};"
    ...
    "{"
        "if(el == document)"
            "el = document.body;"
        "if(typeof(el.outerHTML) != 'undefined' && el.outerHTML.indexOf(mask)<0 && typeof(el.innerHTML) != 'undefined' && el.innerHTML.indexOf(mask)<0)"
            "return [];"
        ...
        "try{"
            ...
            "{"
                ...
                "{"
                    ...
                    "{"
                        "var found = false;"
                        "for(var j = 0;j<res.childElementCount;j++)"
                        "{"
                            "try"
                            "{"
                ...
                "{"
                    ...
                    "let scrolling_element = _BAS_HIDE(BrowserAutomationStudio_GetScrollingNode)();"
                    "res = document.elementFromPoint(x - scrolling_element.scrollLeft,y - scrolling_element.scrollTop);"
                "}"
                "if(!res)"
                    "return null;"
            "}"
