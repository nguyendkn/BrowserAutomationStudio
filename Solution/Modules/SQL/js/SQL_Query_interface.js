...
WHERE age IS NULL</pre>
		...
		Пример #6.1. Получить все записи из таблицы "users", в которых поле "name" равно значению из переменной [[NAME]].<br/><code>Параметр "Параметризировать переменные и ресурсы" должен быть включен, так как переменная не экранированы.</code>
		<pre>SELECT * FROM users
WHERE name = [[NAME]]</pre>

		Пример #6.2. Получить все записи из таблицы "users", в которых поле "name" равно значению из переменной [[NAME]].<br/><code>Параметр "Параметризировать переменные и ресурсы" должен быть отключен, так как переменная уже экранированы.</code>
		<pre>SELECT * FROM users
WHERE name = '[[NAME]]'</pre>
	</span>
</script>