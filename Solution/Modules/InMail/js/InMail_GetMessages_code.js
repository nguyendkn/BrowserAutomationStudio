...
_call_function(_InMail.getMessages, {uids: (<%= uids %>)<%if(body.length){%>, body: ([<%= body.map(function(el){return "'" + el[0] + "'"}).join(', ') %>])<%}%><%if(headers.length){%>, headers: ([<%= headers.map(function(el){return "'" + el[0] + "'"}).join(', ') %>])<%}%><%if(size){%>, size: (<%= size %>)<%}%><%if(attachnames){%>, attachnames: (<%= attachnames %>)<%}%><%if(attachments){%>, attachments: (<%= attachmentsMask %>)<%}%><%if(markSeen){%>, markSeen: (<%= markSeen %>)<%}%><%if(box!=='""'){%>, box: (<%= box %>)<%}%>})!
...
<%if(attachnames){%><%= saveAttachnames %> = messageData["attachnames"];<%}%>
