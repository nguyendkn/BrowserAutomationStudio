    ...
    while(index<len){
        ...
        if(seps.indexOf(c) > -1){
            if(InsideQuotes){
                ...
                res.push(convert ? _convert_type(CurrentElement) : CurrentElement);
                CurrentElement = '';
                continue;
            }
        };
...
function _regexp_extract_validation(mode, type){
	if(_is_nilb(_STR_REGEXP_EV)){
		...
		var file = "(file:\\/\\/\\/)([a-z]+:(\\/|\\\\)+)?([\\w.]+([\\/\\\\]?)+)+";
		var phone = "(?:(?:(?:(\\+)?\\d{1,3}[-\\040.]?)?\\(?\\d{3}\\)?[-\\040.]?\\d{3}[-\\040.]?\\d{4})|(?:(\\+)(?:9[976]\\d|8[987530]\\d|6[987]\\d|5[90]\\d|42\\d|3[875]\\d|2[98654321]\\d|9[8543210]|8[6421]|6[6543210]|5[87654321]|4[987654310]|3[9643210]|2[70]|7|1)[-\\040.]?(?:\\d[-\\040.]?){6,12}\\d+))([,;]+[0-9]+#?)*";
...
function _query_string_decode(qs, sep, eq, options){
	...
	for(var i = 0; i < len; ++i){
		...
		v = _convert_type(decodeURIComponent(vstr));
...
function _parse_ua(uastring, extensions){
	...
	var regexes = {
		...
		device: [
			...
			[
				new RegExp("(kf[A-z]+)\sbuild\/.+silk\\/", "i")
			],
			[
				MODEL,
				[VENDOR, 'Amazon'],
			...
			[
				new RegExp("(sony)\\s(tablet\\s[ps])\\sbuild\\/", "i"),
				new RegExp("(sony)?(?:sgp.+)\\sbuild\\/", "i")
			],
			[
				[VENDOR, 'Sony'],
				[MODEL, 'Xperia Tablet'],
			...
			[
				new RegExp("android.+\\s(mz60\\d|xoom[\\s2]{0,2})\\sbuild\\/", "i")
			],
			[
				MODEL,
				[VENDOR, 'Motorola'],
