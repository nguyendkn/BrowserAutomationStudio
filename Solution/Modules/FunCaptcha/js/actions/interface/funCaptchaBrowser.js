...
<div class="tooltipinternal">
  ...
  <div class="tr tooltip-paragraph-fold">This action does not require you to search for the necessary information to solve the <code>FunCaptcha</code> manually, everything happens automatically without your interaction using the source code of the page and other data.</div>
  <div class="tr tooltip-paragraph-fold">In most cases, you also do not need to submit the solution of the captcha on the site. The action automatically looks for the <code>FunCaptcha</code> validation function in the page code and executes it using the response from the captcha solving service. The exception may be cases where the site does not use such functions - in such situations you will need to proceed manually. For example, press the submit form button or perform another similar action.</div>
  <div class="tr tooltip-paragraph-fold"><code>FunCaptcha</code> is solved on the service worker's computer. This means that the site with the captcha can potentially check the IP of the person who solves it and put this IP in the blacklist. There is a special setting <code>Send current proxy to service</code> (under the advanced settings). If you use this option, the worker will solve the captcha through the proxy server that was set using the <code>Proxy</code> action from the browser actions section.</div>
  <div class="tr tooltip-paragraph-fold">When using a proxy, always fill in the field <code>User-Agent</code>, or use the default value, otherwise the captcha solving service can give an error. If this parameter is specified, the service worker will solve the captcha through the given User-Agent, just like it happens with proxies.</div>
