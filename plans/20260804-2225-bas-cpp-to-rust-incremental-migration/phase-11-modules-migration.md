...
- `ImageProcessing`'s WinAPI/GDI calls go through `bas-platform` if Phase 2 has run (otherwise the `platform` module inside `bas-modules` handles it; same reuse discipline as Phase 6/8), extended with a GDI-specific trait arm if `bas-platform`'s existing set doesn't already cover it — same reuse discipline as Phase 6/8, not a fresh `windows-rs` wrap.
...
## Related Code Files
- `Solution/Modules/` — modify: each of the 41 subdirectories (Archive, Checksum, Clipboard, Database, DateTime, Excel, FileSystem, FTP, ImageProcessing, JSON, List, Path, PhoneVerification, Processes, Profiles, ReCaptcha, RegularExpression, Resources, ScriptStats, SQL, String, Telegram, Timezones, URL, etc.) replaced with a thin registration shim calling `bas-modules`, or (preferred, per Phase 10's design) directly retired in favor of native Rust registration.
- `Solution/FastExecuteScript/` — modify: replaced with a thin binary shim calling `bas-fastexecute`.
- **Create:** `rust/bas-modules/{Cargo.toml,src/lib.rs,src/<40 domain modules>.rs,src/platform.rs}`.
- **Create:** `rust/bas-fastexecute/{Cargo.toml,src/main.rs}`.
- **Create:** `Solution/Tests/ModulesCharacterization/golden/<per-module>/` — per-Module golden fixtures.
