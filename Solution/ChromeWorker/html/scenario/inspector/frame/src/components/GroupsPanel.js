  ...
  template: /*html*/`
    <div class="app-panel">
      <panel-toolbar :sortings.sync="sortings" :filters.sync="filters" :query.sync="query">
        <template #controls>
          <button type="button" style="border-right-color: transparent;" @click="$refs.view.addGroup()">
      ...
      <div v-else class="app-panel-content">
        <group-list ref="view" :data="data" :sortings="activeSortings" :filters="activeFilters" :query="query" />
      </div>
    </div>
  `
};
