...
if(serverUrl["original"].length > 0){
	if(customSite["original"].length == 0){
		Invalid(tr("The parameter \"") + tr("Custom site") + tr("\" is not specified"));
		return;
	};
	if(customCountry["original"].length == 0){
		Invalid(tr("The parameter \"") + tr("Custom country") + tr("\" is not specified"));
		return;
	};
};
