...
std::string JavaScriptExtensions::GetBasicExtension(bool IsRecord)
{
    ...
    "{"
        ...
        "if(el)"
        "{"
            "if(el.tagName.toLowerCase()=='input')element_type='editable';"
            "if(el.tagName.toLowerCase()=='a'){element_type='link'; element_url = el.getAttribute('href'); if(!element_url)element_url='' else element_url = new URL(element_url, document.baseURI).href;};"
            "if(el.tagName.toLowerCase()=='img'){element_type='media'; element_url = el.getAttribute('src'); if(!element_url)element_url='' else element_url = new URL(element_url, document.baseURI).href;;}"
        "}"
        "element_selection = window.getSelection().toString();"
        "return JSON.stringify({'type': element_type,'url': element_url,'selection': element_selection});"
    "};"
