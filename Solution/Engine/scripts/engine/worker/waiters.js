...
function wait_content_visible()
{
    ...
    }else
    {
        wait(tr("Failed to wait for content ") + JSON.stringify(text_success),function(){
            _set_result(false);
            waiter_create_match_path(_arguments()).script("document.readyState!='loading' && Math.round(self.getBoundingClientRect().height) > 0 && Math.round(self.getBoundingClientRect().width) > 0&& window.getComputedStyle(self)['display']!='none'&&window.getComputedStyle(self)['visibility'] != 'hidden'",function(){
...
function wait_css_visible()
{
    ...
    }else
    {
        ...
        wait(tr("Failed to wait for css ") + JSON.stringify(text_success), function(){
            _set_result(false);
            waiter_create_css_path(_arguments()).script("document.readyState!='loading' && Math.round(self.getBoundingClientRect().height) > 0 && Math.round(self.getBoundingClientRect().width) > 0&& window.getComputedStyle(self)['display']!='none'&&window.getComputedStyle(self)['visibility'] != 'hidden'",function(){
               _set_result(_result().indexOf("true")>=0);
            })
        },text_success,func);
    }
    ...
    }else
    {
        ...
        wait(tr("Failed to wait for element ") + text_success, function(){
            if(BROWSERAUTOMATIONSTUDIO_WAIT_ITERATION == 8)
                _info(tr("Waiting for element ") + text_success)
            _set_result(false);
            get_element_selector(_arguments()).script("document.readyState!='loading' && Math.round(self.getBoundingClientRect().height) > 0 && Math.round(self.getBoundingClientRect().width) > 0&& window.getComputedStyle(self)['display']!='none'&&window.getComputedStyle(self)['visibility'] != 'hidden'",function(){
               _set_result(_result().indexOf("true")>=0);
            })
        },text_success,func);
    }
