_InMail.imap = _InMail.assignApi(function(autoConfig, host, port, encrypt, username, password, folder){
	...
	this.buildSearchQuery = function(options, extensions, info, isOrChild){
		...
		for(var i = 0, len = options.length; i < len; ++i){
			...
			if(criteria === 'OR'){
				...
				searchargs += api.buildSearchQuery(args[0], extensions, info, true);
				searchargs += ') (';
				searchargs += api.buildSearchQuery(args[1], extensions, info, true);
				searchargs += ')';
			}else{
				if(criteria[0] === '!'){
					modifier += 'NOT ';
				...
				switch (criteria) {
					...
					case 'HEADER':
						...
						val = api.buildString(args[1]);
						if(info && val[0] === '{'){
							info.hasUTF8 = true;
						};
						searchargs += modifier + criteria + ' "' + api.escape('' + args[0]) + '" ' + val;
						break;
					case 'UID':
						if(!args){
							throw new Error('Incorrect number of arguments for search option: ' + criteria);
