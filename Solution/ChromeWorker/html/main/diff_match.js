(function () {
      ...
      while (1) {
        if (a[d].hasOwnProperty ? a[d].hasOwnProperty(x - 1 + ',' + y) : a[d][x - 1 + ',' + y] !== undefined) {
          ...
          if (f === -1) {
            ...
            e.unshift([-1, b.charAt(x)]);
          }
          f = -1;
          break;
        } else if (
          a[d].hasOwnProperty ? a[d].hasOwnProperty(x + ',' + (y - 1)) : a[d][x + ',' + (y - 1)] !== undefined
        ) {
          y--;
          if (f === 1) {
            e[0][1] = c.charAt(y) + e[0][1];
          } else {
            e.unshift([1, c.charAt(y)]);
          }
          f = 1;
          break;
        } else {
      ...
      switch (a[b][0]) {
        ...
        case 0:
          if (c !== 0 || d !== 0) {
            ...
            if (c === 0) {
              a.splice(b - c - d, c + d, [1, f]);
            } else if (d === 0) {
              a.splice(b - c - d, c + d, [-1, e]);
            } else {
  ...
  _BAS_HIDE(diff_match_patch).prototype.diff_xIndex = function (a, b) {
    var c = 0;
    var d = 0;
    var e = 0;
    var f = 0;
    ...
    if (a.length != x && a[x][0] === -1) {
      return f;
    }
    return f + (b - e);
  };
  _BAS_HIDE(diff_match_patch).prototype.diff_prettyHtml = function (a) {
    var b = [];
    var i = 0;
    for (var x = 0; x < a.length; x++) {
      var c = a[x][0];
  ...
  _BAS_HIDE(diff_match_patch).prototype.diff_text1 = function (a) {
    var b = [];
    for (var x = 0; x < a.length; x++) {
      if (a[x][0] !== 1) {
        b[x] = a[x][1];
  ...
  _BAS_HIDE(diff_match_patch).prototype.diff_text2 = function (a) {
    var b = [];
    for (var x = 0; x < a.length; x++) {
      if (a[x][0] !== -1) {
        b[x] = a[x][1];
  ...
  _BAS_HIDE(diff_match_patch).prototype.diff_toDelta = function (a) {
    var b = [];
    for (var x = 0; x < a.length; x++) {
      switch (a[x][0]) {
        case 1:
  ...
  _BAS_HIDE(diff_match_patch).prototype.diff_fromDelta = function (a, b) {
    var c = [];
    var d = 0;
    var e = 0;
    b = b.replace(/%00/g, '\0');
  ...
  _BAS_HIDE(diff_match_patch).prototype.match_main = function (a, b, c) {
  ...
  _BAS_HIDE(diff_match_patch).prototype.match_bitap = function (a, b, c) {
  ...
  _BAS_HIDE(diff_match_patch).prototype.match_alphabet = function (a) {
    var s = {};
  ...
  _BAS_HIDE(diff_match_patch).prototype.patch_addContext = function (a, b) {
  ...
  _BAS_HIDE(diff_match_patch).prototype.patch_make = function (a, b, c) {
    var d, text2, diffs;
  ...
  _BAS_HIDE(diff_match_patch).prototype.patch_apply = function (a, b) {
    if (a.length == 0) {
      return [b, []];
    }
    var c = [];
  ...
  _BAS_HIDE(diff_match_patch).prototype.patch_addPadding = function (a) {
    var b = '';
    ...
    c = a[a.length - 1];
    d = c.diffs;
    if (d.length == 0 || d[d.length - 1][0] != 0) {
      d.push([0, b]);
      c.length1 += b.length;
      c.length2 += b.length;
  ...
  _BAS_HIDE(diff_match_patch).prototype.patch_splitMax = function (a) {
    for (var x = 0; x < a.length; x++) {
      if (a[x].length1 > this.Match_MaxBits) {
        ...
        while (b.diffs.length !== 0) {
          ...
          if (k !== '') {
            g.length1 += k.length;
            g.length2 += k.length;
            if (g.diffs.length !== 0 && g.diffs[g.diffs.length - 1][0] === 0) {
  ...
  _BAS_HIDE(diff_match_patch).prototype.patch_toText = function (a) {
    var b = [];
    for (var x = 0; x < a.length; x++) {
      b[x] = a[x];
    }
  ...
  _BAS_HIDE(diff_match_patch).prototype.patch_fromText = function (a) {
    var b = [];
    if (!a) {
      return b;
    }
